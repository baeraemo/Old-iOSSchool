...
/***************************************************
 ...
 open class func localizedString(forStatusCode statusCode: Int) -> String // 들어온 번호 그대로 입력하면 해당 언어로 잘나옴
...
final class ViewController: UIViewController {
  ...
  // MARK: - Sync method
    ...
    @IBAction private func syncMethod() {
        print("\n---------- [ syncMethod ] ----------\n")
        let url = URL(string: "http://lorempixel.com/860/640/cats/")! //860 640 고양이 이미지 랜덤으로 받기
        DispatchQueue.global().async {
            if let data = try? Data(contentsOf: url) {
                DispatchQueue.main.async { // 이미지는 무조건 main에다가 해야한다
                print("다운로드 완료")
  ...
  @IBAction private func urlEncodingExample() {
    ...
    print(encodedString!) // prints "color-%23708090"     # <- 이게 %23으로 바뀐거다 그작업을 addingpercent이자식이 해준거다
    
    // URL To String
    let url = URL(string: "https://example.com/#color-%23708090")!
    let components = URLComponents(url: url, resolvingAgainstBaseURL: true)!
    let fragment = components.fragment!  //위에 만들어놓은 %23을 다시 꺼내놓는거
    print(fragment) // prints "color-#708090"
    
    // nil   인터넷 주소창 보면 한글 이부분이 1%%dd#%%ds이런식으로 되어있다 지금도 한글 이 글짜를 저런식으로 바꾸고자한다
    ...
    let queryEncodedStr = str.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)! // 이작업을 해야 한글이 바뀐다 query부분이 한글이 있어서 변환하고자 하는거니까 urlQueryAllowed를 쓰는거다
    let convertedURL = URL(string: queryEncodedStr)!
    print(convertedURL)
    
    //연습해버리기 순서대로 진행해버리기
    let str1 = "https://search.naver.com/search.한r글?query=한글"
    let queryEncodedStr1 = str1.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!
    let queryEncodedStr2 = queryEncodedStr1.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed)!
    let convertedURL1 = URL(string: queryEncodedStr2)!
    print(convertedURL1)
    
    
    // http://username:password@www.example.com/index.html?key=1&value=2#frag
    // CharacterSet.urlUserAllowed    <----user :로 구분
    // CharacterSet.urlPasswordAllowed  <-----password
    // CharacterSet.urlHostAllowed <----주소 시작부분은 @이로 구분 www.
    // CharacterSet.urlPathAllowed < --- index
    // CharacterSet.urlQueryAllowed <-- ?부터 #까지
    // CharacterSet.urlFragmentAllowed <--#부터 frag
  ...
  @IBAction private func sessionConfig(_ sender: Any) {
    ...
    _ = URLSessionConfiguration.default  // 거의 이거로 씀
    _ = URLSessionConfiguration.ephemeral
    _ = URLSessionConfiguration.background(withIdentifier: "kr.giftbot.example.backgroundConfig") // 백그라운드는 아이덴티가있어서 구분하기 때문에 재사용이 불가능
    
    //deepcopy는 복사시 1,2 1,2가 나오고 2,2로 바꾸면 복사한 값 1,2 => 2,2로 된다
    //shallowcopy 1,21,2 => 2,2 2,2 같이바뀜
    ...
    //해당 숫자가왔는데 뭔지 모를때는 그냥 찍어보면 안다
    print(HTTPURLResponse.localizedString(forStatusCode: 200)) // no error
    print(HTTPURLResponse.localizedString(forStatusCode: 404)) // not found
...
//      DispatchQueue.main.async {
    ...
    task?.resume() //completion작업이 끝났을때 알려주는 부분
  }
  ...
  func makeGetCall() {
    ...
    //밑에 3줄은 사실 없어도 상관없음 특별히 뭔가를 적용한게 아님
    let urlRequest = URLRequest(url: url)
    let config = URLSessionConfiguration.default
    let session = URLSession(configuration: config)
    
    //URLSession.shared.dataTask 해도 상관없음
  ...
  func makePostCall() {
    ...
    todosUrlRequest.httpMethod = "POST" // 기본이GET인데 post로 바꾼것
    todosUrlRequest.httpBody = jsonTodo //head랑 body가 있는데 body에다가 했다
    ...
    //post가 되고 난 값 delete가 되고 난 값들이 들어오는 거다
  func makeDeleteCall() {
    let firstTodoEndpoint = "https://jsonplaceholder.typicode.com/todos/1"
    var firstTodoUrlRequest = URLRequest(url: URL(string: firstTodoEndpoint)!)
    firstTodoUrlRequest.httpMethod = "DELETE" // put catch get post 이렇게 4개가있는데 서버에서 구현해줘야하가능한것 delete 인데 서버만드는 사람이 get 기능을 만들어 놓을 수도 있다
    // firstTodoUrlRequest.setValue("인증ㅋ", forHTTPHeaderField: "이름?") 이런 인증을 거쳐야 set delete등 서버에 권한을 얻을 수 있게 서버관리자가 인증하고쓰라고 하는경우가 있다
