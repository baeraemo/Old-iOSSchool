...
let swiftString = Swift.String.init("initString") // <- swift에 속한 스트링 그런 의미라 안써도 무방
let swiftStringSelf = Swift.String.self // 인스턴스
let swiftStringType = type(of: swiftString) // 클래스   //<--- 이녀석의 차이
let swiftStringMetatype = type(of: Swift.String.self)    //<--- 이녀석과
